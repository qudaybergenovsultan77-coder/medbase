...
export default function PayModal({ file, onClose, onSuccess }: Props) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // 1. Chekni Cloudinary ga yuklash
      ...
      <div className="bg-white rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-xl">
        <div className="p-6">
              ...
              {chekFile ? (
                <div className="flex items-center justify-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <div className="text-xs text-emerald-500">{(chekFile.size / 1024 / 1024).toFixed(1)} MB · O'zgartirish uchun bosing</div>
                  </div>
                </div>
              ) : (
                <div>
          ...
          <div className="flex gap-2">
            <button onClick={onClose} disabled={loading} className="flex-1 btn-secondary py-2.5">Bekor qilish</button>
            <button onClick={handleSubmit} disabled={!chekFile || loading} className="flex-1 btn-primary py-2.5">
